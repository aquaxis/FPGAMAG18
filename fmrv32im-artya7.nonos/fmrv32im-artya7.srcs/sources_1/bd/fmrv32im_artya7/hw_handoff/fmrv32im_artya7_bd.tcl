...
CONFIG.NUM_PORTS {1} \
  ...
  # Create instance: uart_and_gpio, and set properties
  set uart_and_gpio [ create_bd_cell -type ip -vlnv user.org:user:fmrv32im_axis_uart:1.0 uart_and_gpio ]

  set_property -dict [ list \
CONFIG.NUM_READ_OUTSTANDING {1} \
CONFIG.NUM_WRITE_OUTSTANDING {1} \
 ] [get_bd_intf_pins /uart_and_gpio/S_AXI]

  # Create interface connections
  connect_bd_intf_net -intf_net axi_lite_master_M_AXI [get_bd_intf_pins axi_lite_master/M_AXI] [get_bd_intf_pins uart_and_gpio/S_AXI]
  ...
  connect_bd_intf_net -intf_net uart_and_gpio_GPIO [get_bd_intf_ports GPIO] [get_bd_intf_pins uart_and_gpio/GPIO]
  connect_bd_intf_net -intf_net uart_and_gpio_UART [get_bd_intf_ports UART] [get_bd_intf_pins uart_and_gpio/UART]

  # Create port connections
  connect_bd_net -net CLK_1 [get_bd_ports CLK100MHZ] [get_bd_pins axi_lite_master/CLK] [get_bd_pins fmrv32im_core/CLK] [get_bd_pins uart_and_gpio/CLK]
  connect_bd_net -net High_dout [get_bd_pins High/dout] [get_bd_pins axi_lite_master/RST_N] [get_bd_pins fmrv32im_core/RST_N] [get_bd_pins uart_and_gpio/RST_N]
  connect_bd_net -net concat_dout [get_bd_pins concat/dout] [get_bd_pins fmrv32im_core/INT_IN]
  connect_bd_net -net uart_and_gpio_INTERRUPT [get_bd_pins concat/In0] [get_bd_pins uart_and_gpio/INTERRUPT]
